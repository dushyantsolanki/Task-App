...
const BigCalendar = () => {
  ...
  const openAddEventSheet = (dateStr: any) => {
    ...
    const startDateTime = dateStr
      ? moment.tz(dateStr, 'Asia/Kolkata').format('YYYY-MM-DDTHH:mm')
      : '';
    const endDate = dateStr
      ? moment.tz(dateStr, 'Asia/Kolkata').add(1, 'hour')
      : now.add(1, 'hour');
  ...
  const openEditEventSheet = (event: any) => {
    ...
    const endDate = event.end
      ? moment.tz(event.end, 'Asia/Kolkata')
      : startDate.clone().add(1, 'hour');
    const selectedColor =
      colorOptions.find((c) => c.color === event.backgroundColor)?.value || 'blue';
    ...
    try {
      ...
      if (editingEvent) {
        ...
        const response = await AxiousInstance.put(
          `/calendar/${editingEvent?.extendedProps?._id}`,
          eventData,
        );
        if (response.status === 200) {
      ...
      <div className="p-3 md:p-6">
        ...
        <div className="mb-4 flex items-center justify-between md:mb-6">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <h2 className="text-3xl md:text-4xl">{getMonthYear()}</h2>
          </div>
                ...
                <div className="grid gap-4 px-4 py-4">
                  ...
                  <div className="grid gap-2">
                    ...
                    {formik.touched.start && formik.errors.start && (
                      <span className="text-sm text-red-500">{formik.errors.start}</span>
                    )}
                  </div>
                  ...
                  <div className="grid gap-2">
                    ...
                    {formik.touched.end && formik.errors.end && (
                      <span className="text-sm text-red-500">{formik.errors.end}</span>
                    )}
                  </div>
                  ...
                  <div className="grid gap-2">
                    ...
                    >
                      <SelectTrigger
                        className={`${formik.touched.color && formik.errors.color ? 'border-red-500' : ''} w-full py-[21px]`}
                      >
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    {formik.touched.color && formik.errors.color && (
                      <span className="text-sm text-red-500">{formik.errors.color}</span>
                    )}
                  </div>
                  ...
                  <div className="mt-4 flex justify-center gap-2">
                    ...
                    <Button onClick={formik.handleSubmit as any} disabled={formik.isSubmitting}>
        ...
        <div className="border-border overflow-hidden rounded-lg border">
          <FullCalendar
            ...
            eventContent={(eventInfo) => {
              ...
              return (
                <div className="flex w-full items-center gap-1.5 !py-1.5 !pr-4 !pl-1">
                  <span
                    className="h-2.5 w-2.5 rounded-full"
                    style={{ backgroundColor: dotColor }}
                  ></span>
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs leading-tight font-medium !text-[var(--foreground)]">
                      {eventInfo.event.title}
                    </span>
                    <span className="text-xs leading-tight !text-[var(--foreground)] opacity-75">
                      {moment.tz(eventInfo.event.start, 'Asia/Kolkata').format('h:mm A')}{' '}
                      {/* Display time in IST */}
                    </span>
                  </div>
                </div>

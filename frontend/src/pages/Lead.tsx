...
function Lead() {
      ...
      cell: ({ row }) => {
        return (
          <Avatar className="h-11 w-11 border">
            <AvatarImage
              src={
                row.original?.createdBy?.avatar?.startsWith('https://')
                  ? row.original?.createdBy?.avatar
                  : import.meta.env.VITE_IMAGE_BASE_URL + row.original?.createdBy?.avatar
              }
      ...
      cell: ({ row }) => (
        <div className="flex items-center space-x-2">
          <div>
            ...
            {row.original.domain && (
              <div className="text-muted-foreground text-xs">{row.original.domain}</div>
            )}
          </div>
        </div>
      ),
...

    {
      ...
      cell: ({ row }) => (
        <div className="space-y-1">
          {row.original.phone && <div className="text-xs">{row.original.phone}</div>}
          {row.original.emails?.length > 0 && (
            <div className="text-muted-foreground text-xs">{row.original.emails[0]}</div>
          )}
  ...
  return (
    <>
      <SEO
        title="TaskMate | AI Lead Automation"
        description="Generate leads efficiently with TaskMate AI. Streamline your lead automation workflow."
        ...
        <div className="mb-6">
          <div className="flex w-full items-center justify-between gap-4 sm:w-auto">
            <div className="flex gap-4">
              <Button onClick={() => setIsModalOpen(true)} className="flex h-11 items-center gap-2">

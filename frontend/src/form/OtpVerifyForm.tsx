...
export default function OtpVerifyForm({
  ...
  const handleOTPVerification = async (values: string) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL || 'https://task-mate-full-stack.onrender.com/api/v1'}/auth/verify-otp`,
        { otp: values, email },
      );
  ...
  const handleResendOtp = async () => {
    if (canResend) {
      ...
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_API_BASE_URL || 'https://task-mate-full-stack.onrender.com/api/v1'}/auth/resend-otp`,

...
export default function LoginForm({ className, ...props }: React.ComponentPropsWithoutRef<'form'>) {
  ...
  const handleSubmit = async (values: any) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL || 'https://task-mate-full-stack.onrender.com/api/v1'}/auth/login`,
        values,
        { withCredentials: true },
      );
  ...
  return (
    <>
      <SEO
        title="TaskMate | Login"
        description="Login to TaskMate to manage your tasks efficiently and collaborate with your team."

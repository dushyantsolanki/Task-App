...
export default function ForgotPasswordFlow({
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_API_BASE_URL || 'https://task-mate-full-stack.onrender.com/api/v1'}/auth/forgot-password`,
          values,
        );
  ...
  const passwordFormik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_API_BASE_URL || 'https://task-mate-full-stack.onrender.com/api/v1'}/auth/forgot-password/reset-password`,
          { email, otp, newPassword: values.confirmPassword },
        );
  ...
  const handleOtpChange = async (value: string) => {
    ...
    if (value.length === otpLength) {
      ...
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_API_BASE_URL || 'https://task-mate-full-stack.onrender.com/api/v1'}/auth/forgot-password/verify-otp`,
          { email, otp: value },
        );
  ...
  const renderOtpSlots = () => {
    ...
    for (let i = 0; i < otpLength; i += groupSize) {
      ...
      slots.push(<InputOTPGroup key={i}>{group}</InputOTPGroup>);
        ...
        <FormikProvider value={emailFormik}>
          <Form className="flex flex-col gap-6">
            ...
            <div className="grid gap-6">
              ...
              <Button type="submit" className="w-full" disabled={isLoading || !emailFormik.isValid}>
      ...
      <div className={cn('flex flex-col gap-6', className)} {...props}>
        ...
        <div className="grid gap-6">
          <div className="flex flex-col gap-4">
            <label className="text-center text-sm font-medium">Enter verification code</label>
            ...
            {isLoading && (
              <p className="text-muted-foreground text-center text-xs">Verifying code...</p>
            )}
          </div>
          ...
          <div className="text-center text-sm">
            <p className="text-muted-foreground">
              ...
              {canResend ? (
                ...
                <span className="font-medium">Resend in {formatCountdown(countdown)}</span>
              )}
            </p>
          </div>

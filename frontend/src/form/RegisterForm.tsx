...
export default function RegisterForm({
  ...
  const handleRegister = async (values: Payload) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL || 'https://task-mate-full-stack.onrender.com/api/v1'}/auth/register`,
        values,
      );
  ...
  return (
    <>
      <SEO
        title="TaskMate | Register"
        description="Create your TaskMate account to manage tasks efficiently and collaborate with your team."
        url="https://taskmate.dushyantportfolio.store/register"
        type="website"
      />
      ...
      <div className="my-6 flex flex-col gap-4 md:flex-row">
        ...
        >
          {loading.google ? (
            <>
              <span className="border-primary mr-2 h-4 w-4 animate-spin rounded-full border-2 border-t-transparent"></span>
              Please wait...
            </>
          ) : (
            <>

...
export default function PasswordResetForm({
  ...
  const handleSendPasswordOTP = async () => {
    try {
      setLoading(true);
      const response = await AxiousInstance.post(`/auth/profile/send-otp?type=forgot_password`, {
        email: user?.email,
  ...
  const handleUpdatePassword = async (values: { newPassword: string; confirmPassword: string }) => {
    ...
    try {
      const response = await AxiousInstance.patch(`/auth/profile/reset-password/${user?.id}`, {
                    ...
                    <>
                      ...
                      {isOTPSent && (
                        <div className="space-y-2">
                          ...
                          <Button
                            type="button"
                            className="h-8 w-full rounded-sm font-semibold transition-colors hover:cursor-pointer"
